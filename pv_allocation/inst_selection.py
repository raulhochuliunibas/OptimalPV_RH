...
def select_AND_adjust_topology(pvalloc_settings, 
    ...
    data_path_def = pvalloc_settings['data_path_def']
    ...
    # import ----------
    with open(f'{data_path_def}/output/pvalloc/topo_egid.json', 'r') as f:
        topo = json.load(f)
    ...
    inst_power = npv_df['FLAECHE'].values[0]  * conv_m2toKWP * pvalloc_settings['algorithm_specs']['capacity_tweak_fact']
    ...
    pred_inst_df = pd.concat([pred_inst_df, npv_pick], ignore_index=True)

    # Adjust topo
    topo[picked_egid]['pv_inst'] = {'inst_TF': True, 'info_source': 'alloc_algorithm', 'BeginOp': f'{m}', 'TotalPower': inst_power}


    return npv_df, inst_power # , picked_uid, picked_combo_uid, pred_inst_df, dfuid_installed_list, topo
