...
if subsample_faster_run == =:
    ...
    if create_data_subsample == 1:
          ...
          checkpoint_to_logfile(f'start creating subsamples')
          ...
          checkpoint_to_logfile(f'finished subsetting roof_kat')
          
          faca_kat_sub =      gpd.sjoin(faca_kat, gm_shp_sub, how="inner", op="within")
          checkpoint_to_logfile(f'*finished subsetting faca_kat')
          bldng_reg_sub =     gpd.sjoin(bldng_reg, gm_shp_sub, how="inner", op="within")
          checkpoint_to_logfile(f'*finished subsetting bldng_reg')
          heatcool_dem_sub =  gpd.sjoin(heatcool_dem, gm_shp_sub, how="inner", op="within")
          checkpoint_to_logfile(f'*finished subsetting heatcool_dem')
          pv_sub =             gpd.sjoin(pv, gm_shp_sub, how="inner", op="within")   
          checkpoint_to_logfile(f'*finished subsetting pv')

          # export subsample to shape files
          checkpoint_to_logfile(f'start exporting subsample shapes')
