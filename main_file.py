...
create_data_subsample = 1         # 0 = do not create SWEET data subsample, 1 = create data subsample
...
#     checkpoint_to_logfile(f'\t\t * finished subset roof_kat_1_2_4_8_19_20', n_tabs = 1)
    ...
    roof_kat_res = roof_kat.loc[roof_kat['SB_OBJEKTART'].isin(cat_sb_object)].copy()
    roof_kat_res['SB_OBJEKTART'].value_counts()
    roof_kat_res.to_file(f'{data_path}/temp_cache/residential_subsample/roof_kat_1.shp')
    checkpoint_to_logfile(f'\t\t * finished subset roof_kat_1', n_tabs = 1)
    ...
    cat_sb_object = [1,] #TODO: check if this is the same as for roof_kat
    faca_kat_res = faca_kat.loc[faca_kat['SB_OBJEKTART'].isin(cat_sb_object)].copy()
    faca_kat_res.to_file(f'{data_path}/temp_cache/residential_subsample/faca_kat_1.shp')
    checkpoint_to_logfile(f'\t\t * finished subset faca_kat_1', n_tabs = 1)
    ...
    if create_data_subsample == 1:
     ...
     checkpoint_to_logfile(f'using SWEET SUBSAMPLE(1) for faster run', n_tabs = 1)
