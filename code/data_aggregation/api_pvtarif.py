...
def api_pvtarif_gm_ewr_Mapping(
    ...
    Map_gm_ewr.to_parquet(f'{data_path_def}/input_api/Map_gm_ewr.parquet')
    Map_gm_ewr.to_csv(f'{data_path_def}/input_api/Map_gm_ewr.csv', index=False)
    checkpoint_to_logfile(f'exported Map_gm_ewr from API', log_file_name_def=log_file_name_def, n_tabs_def = 3)

    with open(f'{data_path_def}/input_api/time_stamp.txt', 'w') as f:
...
def api_pvtarif_data(
    ...
    Map_gm_ewr = pd.read_parquet(f'{data_path_def}/input_api/Map_gm_ewr.parquet')
    ...
    pvtarif.to_parquet(f'{data_path_def}/input_api/pvtarif.parquet')
    pvtarif.to_csv(f'{data_path_def}/input_api/pvtarif.csv', index=False)
