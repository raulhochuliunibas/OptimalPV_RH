...
class PVAllocScenario:
    ...
    def __init__(self, 
                 ...
                 T0_prediction      = '2023-01-01 00:00:00',            # start date for the prediction of the future construction capacity
                 months_lookback    = 12,                              # number of months to look back for the prediction of the future construction capacity
                 months_prediction  = 12,                            # number of months to predict the future construction capacity
                 ...
                 # gwr_selection_specs
                 GWRspec_solkat_max_n_partitions    = 10,               # larger number of partitions make all combos of roof partitions practically impossible to calculate
                 GWRspec_solkat_area_per_EGID_range = [2,600],          # for 100kWp inst, need 500m2 roof area => just above the threshold for residential subsidies KLEIV, below 2m2 too small to mount installations
                 ...
                 GWRspec_GBAUJ_minmax               = [1950, 2022],     # GBAUJ_minmax: range of years of construction
                ...
                # weather_specs
                WEAspec_meteo_col_dir_radiation     = 'Basel Direct Shortwave Radiation', 
                WEAspec_meteo_col_diff_radiation    = 'Basel Diffuse Shortwave Radiation',
                WEAspec_meteo_col_temperature       = 'Basel Temperature [2 m elevation corrected]', 
                WEAspec_weather_year                 = 2022,
                WEAspec_radiation_to_pvprod_method  = 'dfuid_ind',          #'flat', 'dfuid_ind'
                WEAspec_rad_rel_loc_max_by          = 'dfuid_specific',     # 'all_HOY', 'dfuid_specific'
                WEAspec_flat_direct_rad_factor      = 1,
                WEAspec_flat_diffuse_rad_factor     = 1,

                # constr_capacity_specs
                CSTRspec_ann_capacity_growth        = 0.05,         # annual growth of installed capacity# each year, X% more PV capacity can be built, 100% in year T0
                CSTRspec_constr_capa_overshoot_fact = 1, 
                CSTRspec_month_constr_capa_tuples   = [(1,  0.06), (2,  0.06), (3,  0.06), (4,  0.06), 
                                                       (5,  0.08), (6,  0.08), (7,  0.08), (8,  0.08), 
                                                       (9, 0.10),  (10, 0.10), (11, 0.12), (12, 0.12),
                                                       ],

                # tech_economic_specs
                TECspec_self_consumption_ifapplicable       = 1,
                TECspec_interest_rate                       = 0.01,
                TECspec_pvtarif_year                        = 2022, 
                TECspec_pvtarif_col                         = ['energy1', 'eco1'],
                TECspec_pvprod_calc_method                  = 'method2.2',
                TECspec_panel_efficiency                    = 0.21,         # XY% Wirkungsgrad PV Modul
                TECspec_inverter_efficiency                 = 0.95,         # XY% Wirkungsgrad Wechselrichter
                TECspec_elecpri_year                        = 2022,
                TECspec_elecpri_category                    = 'H4', 
                TECspec_invst_maturity                      = 25,
                TECspec_kWpeak_per_m2                       = 0.2,          # A 1m2 area can fit 0.2 kWp of PV Panels, 10kWp per 50m2; ASSUMPTION HECTOR: 300 Wpeak / 1.6 m2
                TECspec_share_roof_area_available           = 1,            # x% of the roof area is effectively available for PV installation  ASSUMPTION HECTOR: 70%¨
                TECspec_max_distance_m_for_EGID_node_matching = 0,          # max distance in meters for matching GWR EGIDs that have no node assignment to the next grid node
                TECspec_kW_range_for_pvinst_cost_estim      =[0 , 61],      # max range 2 kW to 150
                TECspec_estim_pvinst_cost_correctionfactor  = 1,

                # panel_efficiency_specs
                PEFspec_variable_panel_efficiency_TF = True,
                PEFspec_summer_months                = [6,7,8,9],
                PEFspec_hotsummer_hours              = [11, 12, 13, 14, 15, 16, 17,],
                PEFspec_hot_hours_discount           = 0.1,

                # sanitycheck_summary_byEGID_specs
                CHECKspec_egid_list = [                                             # ['3031017','1367570', '3030600',], # '1367570', '245017418'      # known houses in the sample in Laufen
                ...
                CHECKspec_n_EGIDs_of_alloc_algorithm        = 20,
                CHECKspec_n_iterations_before_sanitycheck   = 1,

    
                # PART II: settings for MC algorithm --------------------
                # MC_loop_specs
                MCspec_montecarlo_iterations      = 1,
                MCspec_fresh_initial_files        = ['topo_egid.json', 'months_prediction.parquet', 
                                                    'gridprem_ts.parquet', 'constrcapa.parquet', 
                                                    'dsonodes_df.parquet'],  #'gridnode_df.parquet',
                MCspec_keep_files_month_iter_TF   = True,
                MCspec_keep_files_month_iter_max  = 9999999999,
                MCspec_keep_files_month_iter_list = ['topo_egid.json', 'npv_df.parquet', 'pred_inst_df.parquet', 'gridprem_ts.parquet',], 

                # algorithm_specs
                ALGOspec_inst_selection_method              = 'random',   # random, prob_weighted_npv, max_npv 
                ...
                ALGOspec_npv_update_groupby_cols_topo_aggdf  =  ['EGID', 'df_uid', 'grid_node', 'bfs', 
                                                                ...
                                                                'NEIGUNG','STROMERTRAG'], 
                ALGOspec_npv_update_agg_cols_topo_aggdf     = {'pvprod_kW': 'sum', 'demand_kW': 'sum', 
                                                               'selfconsum_kW': 'sum', 'netdemand_kW': 'sum', 
                                                               'netfeedin_kW': 'sum', 'econ_inc_chf': 'sum', 
                                                               'econ_spend_chf': 'sum'}, 
                ALGOspec_tweak_constr_capacity_fact         = 1,
                ALGOspec_tweak_npv_calc                     = 1,
                ALGOspec_tweak_npv_excl_elec_demand         = True,
                ALGOspec_tweak_gridnode_df_prod_demand_fact = 1,
                ALGOspec_constr_capa_overshoot_fact         =1, # not in that dir but should be a single tweak factor dict. 
                
                # gridprem_adjustment_specs
                GRIDspec_tier_description = 'tier_level: (voltage_threshold, gridprem_Rp_kWh)',
                GRIDspec_power_factor = 1, 
                GRIDspec_perf_factor_1kVA_to_XkW = 0.8,
                GRIDspec_colnames = ['tier_level', 'used_node_capa_rate', 'gridprem_Rp_kWh'],
                GRIDspec_tiers = { 1: [0.7, 1], 2: [0.8,  3],  3: [0.85, 5], 
...
pvalloc_scen_list = [
    ...
    # pvalloc_BLSOsml_10y_f2013_5mc_meth2.2_npv        
    PVAllocScenario(
        name_dir_export    = 'pvalloc_BLSOsml_10y_f2013_5mc_meth2.2_npv',
        name_dir_import    = 'preprep_BL_22to23_extSolkatEGID',
        show_debug_prints  = True,
        bfs_numbers        = [        
    ...
    # pvalloc_BLSOsml_10y_f2013_5mc_meth2.2_rnd
    PVAllocScenario(
        name_dir_export    = 'pvalloc_BLSOsml_10y_f2013_5mc_meth2.2_rnd',
        name_dir_import    = 'preprep_BL_22to23_extSolkatEGID',
        show_debug_prints  = True,
        bfs_numbers        = [        
            2768, 2761, 2772, 2785, 2787,
    ...
    # pvalloc_BLSOsml_10y_f2013_5mc_meth2.2_max
    PVAllocScenario(
        name_dir_export    = 'pvalloc_BLSOsml_10y_f2013_5mc_meth2.2_max',
        name_dir_import    = 'preprep_BL_22to23_extSolkatEGID',
        show_debug_prints  = True,
        bfs_numbers        = [        
            2768, 2761, 2772, 2785, 2787,
    ...
    # pvalloc_BLSOsml_15y_f2008_5mc_meth2.2_npv        
    PVAllocScenario(
        name_dir_export    = 'pvalloc_BLSOsml_15y_f2008_5mc_meth2.2_npv',
        name_dir_import    = 'preprep_BL_22to23_extSolkatEGID',
        show_debug_prints  = True,
        bfs_numbers        = [        
            2768, 2761, 2772, 2785, 2787,
        ...
        T0_prediction      = '2008-01-01 00:00:00',            # start date for the prediction of the future construction capacity
        months_prediction  = 180,
        GWRspec_GBAUJ_minmax = [1920, 2012],
        ALGOspec_inst_selection_method = 'prob_weighted_npv',
        TECspec_pvprod_calc_method = 'method2.2',
        MCspec_montecarlo_iterations = 5,
        ),
    # pvalloc_BLSOsml_15y_f2008_5mc_meth2.2_rnd
    PVAllocScenario(
        name_dir_export    = 'pvalloc_BLSOsml_15y_f2008_5mc_meth2.2_rnd',
        name_dir_import    = 'preprep_BL_22to23_extSolkatEGID',
        show_debug_prints  = True,
        bfs_numbers        = [        
            2768, 2761, 2772, 2785, 2787,
    ...
    # pvalloc_BLSOsml_15y_f2008_5mc_meth2.2_max
    PVAllocScenario(
        name_dir_export    = 'pvalloc_BLSOsml_15y_f2008_5mc_meth2.2_max',
        name_dir_import    = 'preprep_BL_22to23_extSolkatEGID',
        show_debug_prints  = True,
        bfs_numbers        = [        
            2768, 2761, 2772, 2785, 2787,
