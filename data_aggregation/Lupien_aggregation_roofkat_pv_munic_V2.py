...
df_agg = roof_kat.merge(Map_roof_pv[['SB_UUID', 'xtf_id']], on='SB_UUID', how='left')
df_agg = df_agg.merge(pv[['xtf_id', 'TotalPower', 'InitialPower',  'BeginningOfOperation']], on='xtf_id', how='left')


# transform date to year -------------------------------------------------------
df_agg['BeginningOfOperation'] = pd.to_datetime(df_agg['BeginningOfOperation'])
df_agg['year'] = df_agg['BeginningOfOperation'].dt.year

df_agg_pq = df_agg.copy()
