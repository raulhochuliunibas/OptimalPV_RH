...
for i, m in enumerate(months_prediction):
    ...
    while (constr_built_m < constr_capa_m) & (constr_built_y < constr_capa_y) & (safety_counter < safety_counter_max ):
        ...
        if any([constr_m_TF, constr_y_TF, safety_TF]):
            checkpoint_to_logfile(f' {safety_counter} pv installations allocated', log_name, 1, show_debug_prints)
            safety_counter = 0
            ...
            checkpoint_to_logfile(f' Update gridprem_ts for month {m}, (stamp: {datetime.now()})', log_name, 1, True)
            ...
            # UPDATE NPV DF ------------------------------------------------------------
            print_to_logfile(f'', log_name) if show_debug_prints else None            
            checkpoint_to_logfile(f' Update npv_nopv_df for month {m}, (stamp: {datetime.now()})', log_name, 1, True)
