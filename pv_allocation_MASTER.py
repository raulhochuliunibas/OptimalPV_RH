...
# ALLOCATION LOOP ----------------------------------------------
    for i, m in enumerate(months_prediction):
        ...
        while( (constr_built_m < constr_capa_m) & (constr_built_y < constr_capa_y) & (safety_counter < safety_counter_max) ):
            ...
            if npv_df.shape[0] > 0: 
                npv_df, inst_power = select.select_AND_adjust_topology(pvalloc_settings, npv_df, 
        ...
        if not os.path.exists(f'{data_path}/output/pvalloc_run/interim_predictions'):
            ...
            os.remove(f'{data_path}/output/pvalloc_run/interim_predictions/*')

        npv_df.to_parquet(f'{data_path}/output/pvalloc_run/interim_predictions/npv_df_{m}.parquet')
        pred_inst_df.to_parquet(f'{data_path}/output/pvalloc_run/interim_predictions/pred_inst_df_{m}.parquet')

        with open(f'{data_path}/output/pvalloc_run/interim_predictions/topo_{m}.json', 'w') as f:
            json.dump(topo, f)
