...
def plot(pvalloc_scen_list, 
    ...
    if visual_settings['plot_ind_line_PVproduction'][0]:
        
        checkpoint_to_logfile('plot_ind_line_PVproduction', log_name)
        ...
        for i_scen, scen in enumerate(scen_dir_export_list):
            ...
            gridnode_df_paths = glob.glob(f'{mc_data_path}/pred_gridprem_node_by_M/gridnode_df_*.parquet')
            ...
            for month in prod_month_df['BeginOp_month'].unique():
                ...
                grid_subdf = pd.read_parquet(f'{mc_data_path}/pred_gridprem_node_by_M/gridnode_df_{month_str}.parquet')
            ...
            if visual_settings['save_plot_by_scen_directory']:
                fig.write_html(f'{data_path}/output/visualizations/{scen}/{scen}__plot_ind_line_PVproduction.html')
            else:
                fig.write_html(f'{data_path}/output/visualizations/{scen}__plot_ind_line_PVproduction.html')
